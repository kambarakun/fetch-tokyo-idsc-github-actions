...
- **ログ**: 構造化ログの使用
- **括弧・記号**: **半角括弧 `()` を使用** (Ruff RUF002/RUF003で自動検出)
...
**重要**: このプロジェクトでは、tree形式のコメント位置統一を自動的にチェックするpre-commitフックが設定されています。

**フック名**: `check-tree-comment-alignment`
