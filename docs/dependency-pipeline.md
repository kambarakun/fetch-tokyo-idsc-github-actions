...
- **検査 2 の 3 件** は正常時の実測が 0 件であることに基づく。**cooldown 未満の新しいリリースと major バンプは数えない**。前者は PR がまだ出る時期ではなく、後者は `dependabot.yml` が `version-update:semver-major` を無視するため、どちらも数えると正常時に発火してノイズになる。停止期間中は 5 件まで積み上がっていた
