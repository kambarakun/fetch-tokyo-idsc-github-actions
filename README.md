...
| **最新週次データ** | 2026年第19週 |
| **最新月次データ** | 2026年4月 |
| **最新データ取得日時** | 2026-05-19 06:57 JST |
| **最終データ更新日時** | 2026-05-14 18:26 JST |
...
- **同一疾患は推移グラフと乖離率グラフで同じ色・同じマーカー**で表示
  - 具体的な色・マーカーは表示順 (各チャートのトップ5) によって割り当てが決まるため、新しいデータが入ると変わり得る。重要なのは「両チャートで揃っている」ことであり、絶対数と流行兆候を並べて追跡しやすい
- **乖離率チャートにしか登場しない疾患は別系統の色 (seaborn `Set2`) +別マーカー** (推移チャートとの識別を分けるため)
- **色覚多様性 (CVD) 配慮**: seaborn `colorblind` パレット (Wong / Okabe-Ito 系統) を採用し、色+形状の冗長エンコーディングで色識別が難しい利用者にも個々の疾患を連続トラッキング可能
- **乖離率の定義**: 同週/同月の**過去5年平均**をベースラインとした CDC FluView 流の指標 — `(実測 - ベースライン) / ベースライン × 100` (%)。正の値は平年比で流行傾向、負は減少傾向を示す
...
| データ種別 | 件数 | データ期間 | 欠損 |
|-----------|------|-----------|------|
| 全数週次 | 1,375件 | 2000年第1週-2026年第19週 | なし |
| 定点週次・保健所別 | 1,375件 | 2000年第1週-2026年第19週 | なし |
| 定点週次・年齢群 | 1,375件 | 2000年第1週-2026年第19週 | なし |
| 定点週次・医療圏別 | 1,362件 | 2000年第14週-2026年第19週 | なし |
| 定点週次・性別 | 1,362件 | 2000年第14週-2026年第19週 | なし |
...
<details>
<summary><strong>[csv_format] Inconsistent column count</strong> (8110件)</summary>
...
<details>
<summary><strong>gender_sum_consistency</strong> (1010ファイル, 不整合: 11667件)</summary>
